...
_Current addendum: 2026-07-22. The historical body below is preserved as the
original phase handover. Since that snapshot, voice and Claude daily ceilings
were implemented, all Claude round trips are metered, Resend-compatible SMTP
configuration was documented, the production Webpack build passed, and Marty
approved indicative pricing from `lib/agent/pricing.ts` (rate card plus the
10–30% discount range; exact matrix withheld). Statements below that list
pricing or production email as unresolved are therefore superseded._
...
### Voice — two-way conversation (COMPLETE) — guide Section 7
- ElevenLabs integration: TTS (lib/voice/elevenlabs.ts, streaming, voice=Daniel, flash_v2_5) +
  STT (lib/voice/stt.ts, scribe). Config in lib/voice/config.ts (voiceId swappable for Marty).
- Architecture: YOUR agent is the brain; ElevenLabs is ears+TTS only. Loop: STT -> runAgentTurn
  (full search/validate/BUFFER-AND-GATE) -> validated text -> TTS. NOTHING unvalidated is ever
  spoken. Turn-based by necessity (gate must finish before speaking); made to FEEL continuous.
- Hands-free: VAD auto-listening (no tap-to-talk), barge-in (talk over playback stops it), echo
  handling (don't self-trigger on Daniel's audio), noise-threshold tuning.
- Latency masking: instant safe FILLER clips (fixed phrases, not model output) cover the
  STT+agent gap so no dead air. Voice register = conversational spoken phrasing (shorter, no
  markdown/headers) — sounds like a person, not a document reader.
- UI: three-panel — live transcript (left), glowing Paramount LOGO as orb (center, state-reactive
  aura), referenced projects (right). Rotating status text. Dark navy.
- Formatting stripped: [[case:ID]] tags AND markdown removed from BOTH captions and TTS input
  (Daniel never speaks "asterisk" or tag IDs).
- Doc-gen reachable by voice: generated one-pagers surface in a prominent "READY TO DOWNLOAD"
  area (latest turn's docs), matched to cases by caseId (not fuzzy title), and DOWNLOAD works
  (fixed a local-dev storage URL mismatch — file written to public/uploads but URL must match;
  Vercel Blob URLs work in prod). Captions reveal in sync with speech (fixed a MediaSource
  duration bug that dumped the whole caption early). Wording avoids "above/below".
- Metered: STT seconds + TTS chars per-org into usage dashboard (costRates has ElevenLabs +
  Scribe rates). Generous daily voice ceilings (lib/gating/voiceLimit.ts) — TIGHTEN before launch.
- Gated like chat; voice turns persist as Messages in the same Conversation (unified history,
  shown in transcript viewer, count toward msg cap).
